...
import htmlReady from 'utils/bodyProcessing/htmlReady';
import YoutubePlayer from 'components/elements/common/YoutubePlayer/YoutubePlayer';
import getSanitizeConfig, { noImageText } from 'utils/bodyProcessing/sanitizeConfig';
...
class MarkdownViewer extends Component {
  ...
  shouldComponentUpdate(np, ns) {
    return (
      ...
      ns.forceShowImages !== this.state.forceShowImages
    );
  }

  onAllowNoImage = () => {
    this.setState({ forceShowImages: true });
  };

  render() {
    const { noImage, className, large, highQualityPost } = this.props;
    const { forceShowImages } = this.state;
    ...
    if (this.props.allowDangerousHTML === true) {
      ...
      cleanText = sanitize(
        ...
        getSanitizeConfig({
          large,
          highQualityPost,
          noImage: noImage && !forceShowImages,
        })
      );
    }
    ...
    for (let section of cleanText.split('~~~ embed:')) {
      const match = section.match(/^([A-Za-z0-9_-]+) (youtube|vimeo|coub|ok_video|rutube) ~~~/);
