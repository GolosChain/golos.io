...
export default class HintIcon extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <div className="HintIcon">
        <Icon name="editor/info" className="HintIcon__icon" onClick={this._onClick} />
  ...
  _renderHint() {
    return (
      <div className="HintIcon__bubble" ref={this.bubbleRef}>
  ...
  _onAwayClick = e => {
    if (!this.bubbleRef.current.contains(e.target)) {
      setTimeout(() => {
        if (!this._unmount) {
